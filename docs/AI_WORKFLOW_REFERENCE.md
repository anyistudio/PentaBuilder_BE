...
- LoL PC 必须返回 6 步
- Wild Rift 必须返回 7 步
- 当 `recommendation_count` 小于剩余空槽数时，后续未推荐的空槽必须保持 `null`
...
- `status_evaluation` 必须解释基础数值强弱差异的来源：
  - 英雄本身的自然属性 / 定位 / 技能画像
  - 当前状态，例如已拥有装备数量、金币、符文、K/D 压力、模式和当前时间点
  - 具体已拥有装备带来的属性加成
- `own_kill_frequency_vs_enemies` 必须完整覆盖当前所有敌方英雄，且不能重复
- `enemy_statuses` 也必须完整覆盖当前所有敌方英雄，且不能重复
- 击杀频率不能超过假定对局时长
- 推塔速度表示实际 5v5 对局里的“净推塔进度”，不是只看英雄裸打塔 DPS
- 计算推塔速度时必须先看双方当前装备和 0-10 状态面板带来的实力对比，再把被击杀、被打残回城、没兵线、被压出塔前窗口、缺清线或害怕控制先手等阻碍折算进去
- 即使当前 UI 只展示我方和一个敌方英雄，也默认这是均衡 5v5 对局；其他队友在场且没有重大失误，不能按 1v1 Solo 或自定义打塔测试来估算
- 返回结果里会附上当前参与实体的参数 appendix

prompt 侧的额外上下文：

- 所有 workflow 的 match overview 都会提示：可见英雄是本次分析焦点，但比赛环境默认是均衡 5v5，不是孤立 1v1
- 当前我方塔目标
- 当前每个敌方的塔目标
- 面向 prompt 的 compact 参数附录
- 明确要求模型先估算 0-10 基础数值，再用这些估计支撑击杀频率和推塔速度
- 推塔速度必须从英雄打塔画像出发，再结合装备是否正确、当前战力/生存能力、兵线/清线、被击杀或被迫离塔的概率来修正
- 规则上强调“理由优先锚定当前已拥有装备，再解释装备与英雄机制、对线/团战关系”
- `status_evaluation` 要点名导致数值对比差异的具体来源，例如某件装备提高了魔法攻击、护甲或穿透，或者差距主要来自英雄本身的坦度 / 爆发 / 射程定位
- 若当前装备信息缺失，按“尚未做出第一件核心装”处理
- 若符文信息缺失，按该英雄在当前模式下的默认符文处理
- 不要输出“当前缺少双方已拥有装备、符文和具体出装进度”这类限制性说明，而是直接基于已知信息估算
